...
FIELD_REFERENCE = [
    ...
    (
        ...
        [
            ...
            ["Excel Export", "Downloads a true .xlsx workbook for spreadsheet filtering or analysis.", "Use Excel when staff need to sort, filter, reconcile data, or share a workbook."],
            ["PDF Export", "Downloads a clean printable PDF report snapshot.", "Use PDF for leadership updates, meeting packets, or non-editable sharing."],
        ],
    ),
    (
    ...
    (
        ...
        [
            ...
            ["Activity Log Search", "Search box on Admin Activity Log.", "Use it to find a sponsor, person, record label, user, or summary text."],
            ["Activity Log Area", "Filters activity by app area.", "Use this to focus on People, Sponsors, Gifts, Campaigns, Communications, Admin, Ask, or Templates."],
            ["Activity Log Action", "Filters by the kind of change.", "Use this for created, updated, status changed, sent, scheduled, printed, scanned, activated, or deactivated events."],
            ["Activity Log Date Range", "Limits audit events by when they occurred.", "Use this when reviewing a specific event day, pickup window, or operating week."],
            ["Activity Log Row", "Clickable audit event summary.", "Click a row to see details, including before/after field values when they were captured."],
            ["Export Buttons", "Download report output.", "Use Excel for a .xlsx workbook and PDF for a formatted snapshot. Exports use the rows currently loaded on the screen."],
        ],
    ),
]
    ...
    story.extend(
        [
            ...
            *screenshot("gift-search.png", "Gift Search supports natural-language searching by age, gender, gift type, and other filters."),
            h2("Gift Search"),
            p("Gift Search helps staff or eligible sponsors find gifts to reserve or commit to. The search supports natural-language prompts such as 'girls age 8 to 10 who need coats' and can use Qdrant semantic matching for broader terms such as toys, Batman gifts, or video games when the wishlist uses a more specific description. Search chips show inferred filters and can be cleared. Results are paginated, sorted in natural recipient ID order, and can be exported to PDF or Excel from the upper right of the results container."),
            workflow("Commit a Gift to a Sponsor", ["Open Gift Search or Gift Status.", "Select the gift and open the commit action.", "Search for the sponsor by name, email, phone, or organization.", "Click the sponsor result so the selected sponsor appears below the search bar.", "Add optional notes for the commitment.", "Choose Commit."]),
            workflow("Review or Release a Committed Gift", ["Click a recipient in Gift Search to review recipient and gift details.", "Click a sponsor name on a committed gift to open the sponsor drawer.", "To make a committed gift available again, choose Release and confirm the sponsor shown in the modal."]),
            h2("Gift Operations"),
            ...
            h2("Gift Pool"),
            p("The Gift Pool is for donated gifts that are not already promised to a specific person. Use it for toy-drive overflow, bulk coats, gift cards, emergency inventory, senior ministry donations, or other extra items. Staff enter the donated items once, then assign the available quantity to recipient wishlist items later."),
            bullets([
                "The top cards separate row counts from physical item counts. Inventory Lines counts table rows. Total Items, Available Items, and Assigned Items count actual physical quantities.",
                "Use Add Inventory to enter donated goods, quantity, category, size, age fit, gender fit, condition, source label, and notes.",
                "Click an inventory row to open the assignment drawer.",
                "Review Available, Category, Size, and Fit before assigning so staff know what item they are matching.",
                "Suggested Matches is the normal starting point. It shows fit-ranked recommendations for the selected inventory item.",
                "Needs Gifts is for fairness checks. It shows recipients who do not have any gift covered yet, ordered by family and recipient ID.",
                "Search is for a specific lookup. It stays empty until staff type a recipient ID, family ID, recipient name, gift description, category, or size.",
                "Set the quantity to assign, add optional assignment notes, and choose Assign. The assignment reduces available inventory and updates the recipient wishlist item.",
                "If staff assign inventory to the wrong recipient, use Remove in the Assignments section. Removing asks for confirmation, returns the quantity to available inventory, and reopens the recipient gift if nothing else covers it.",
            ]),
            workflow("Assign Donated Inventory", ["Open Gifts, then Gift Pool.", "Find or filter the donated inventory row.", "Click the row to open assignment details.", "Use Suggested Matches first if staff want the system's best recommendations.", "Use Needs Gifts if staff want to prioritize recipients with no gift covered.", "Use Search if staff already know the recipient ID, family ID, name, gift, category, or size.", "Review the recipient, family ID, gift description, size, category, and needed quantity.", "Enter the quantity to assign. Do not assign more than the recipient needs or more than inventory has available.", "Add notes when the donated item is a substitute or staff need to explain why the item was assigned.", "Choose Assign."]),
            workflow("Remove a Pool Assignment", ["Open Gifts, then Gift Pool.", "Find the inventory row that was assigned incorrectly.", "Click the row to open assignment details.", "In Assignments, find the recipient or wishlist item that should no longer receive that inventory.", "Choose Remove.", "Read the confirmation carefully and confirm the removal.", "The available inventory quantity increases again. The recipient gift returns to open if it is no longer covered by pool inventory."]),
            ...
            *screenshot("ask-blessing-tree.png", "Ask Blessing Tree supports help, navigation, and report-style prompts."),
            ...
            p("Reports appear under People, Sponsors, Gifts, Ask Blessing Tree, and Admin Activity Log. Report screens are designed to be exported to Excel and PDF. Excel is useful for filtering and analysis; PDF is useful for sharing a clean snapshot."),
            data_table(
            ...
            note("Export scope", "Exports use the rows currently loaded on the screen. On paginated screens such as Activity Log, apply the filters and row count you want before exporting."),
            PageBreak(),
            h1("Admin"),
            ...
            *screenshot("admin-user-management.png", "Admin User Management controls users, invitations, active status, and campaign screen access."),
            ...
            h2("Activity Log"),
            p("Activity Log shows a durable history of important changes across the app. Use it to answer questions such as who updated a sponsor, who changed a gift status, who edited campaign settings, who changed user access, or when a communication/template was changed."),
            *screenshot("admin-activity-log.png", "Admin Activity Log shows searchable change history with PDF and Excel exports."),
            bullets([
            ...
            note("Health checks are non-destructive", "The email check verifies configuration only; it does not send mail. The Qdrant Generate Index action rebuilds Ask knowledge and semantic gift search indexes from MySQL."),
            h2("Production Backup, Restore, and Safety"),
            p("Production data lives in RDS MySQL. Qdrant is a rebuildable search index, and Valkey holds operational queue/cache state. Keep RDS automated backups enabled and use manual backups before risky work."),
            bullets([
            ...
            note("Campaign delete safety", "Campaign deletion is app-admin only. The confirmation modal requires the exact campaign name and campaign year and shows the deletion-impact list before the delete button is enabled."),
