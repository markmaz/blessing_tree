...
def build_document() -> None:
    ...
    add_heading(doc, "Calendar Intelligence", level=2)
    add_body(
        doc,
        "Campaign Studio Schedule starts with a Calendar Overview before the full month calendar. This overview is meant to help a campaign manager quickly see what needs attention without reading every calendar item.",
    )
    add_bullets(
        doc,
        [
    ...
    add_workflow(
        doc,
        "Assign Donated Inventory",
        [
            "Open Gifts, then Gift Pool.",
            "Find or filter the donated inventory row.",
            "Click the row to open assignment details.",
            "Use Suggested Matches first if staff want the system's best recommendations.",
            "Use Needs Gifts if staff want to prioritize recipients with no gift covered.",
            "Use Search if staff already know the recipient ID, family ID, name, gift, category, or size.",
            "Review the recipient, family ID, gift description, size, category, and needed quantity.",
            "Enter the quantity to assign. Do not assign more than the recipient needs or more than inventory has available.",
            "Add notes when the donated item is a substitute or staff need to explain why the item was assigned.",
            "Choose Assign.",
        ],
    )
    add_workflow(
        doc,
        "Remove a Pool Assignment",
        [
            "Open Gifts, then Gift Pool.",
            "Find the inventory row that was assigned incorrectly.",
            "Click the row to open assignment details.",
            "In Assignments, find the recipient or wishlist item that should no longer receive that inventory.",
            "Choose Remove.",
            "Read the confirmation carefully and confirm the removal.",
            "The available inventory quantity increases again. The recipient gift returns to open if it is no longer covered by pool inventory.",
        ],
    )
