...
- The Communications section is now template-only and uses a Query Forge-inspired template-builder workspace with a collapsible saved-template rail, heading/text/image content blocks, inline uploads for small embedded images such as maps, and a stronger rendered-preview surface with a merge-field drawer, while communication timing remains in the calendar/scheduler surfaces.
