...
- Rationale: Blessing Tree serves children in family intake flows, adults in care-facility flows, and adults submitted through partner organizations or home-based senior programs. Those are different intake programs, but they still share the same downstream sponsorship, fulfillment, pickup, and reporting pipeline.
- Consequence: parents/guardians, facility staff, and partner-program coordinators remain contacts, not recipients; `recipient_group` remains the intake container; each actual recipient keeps one wishlist; the schema should evolve from `HOUSEHOLD | INSTITUTION` and `CHILD | ADULT | SENIOR` toward `HOUSEHOLD | CARE_FACILITY | PARTNER_PROGRAM`, `recipient_kind`, and explicit `program_type` values such as `CHILD_FAMILY`, `SENIOR_FACILITY`, and `SENIOR_PARTNER_PROGRAM`; adult recipient address/phone/email fields should be supported where appropriate, especially for partner-program recipients and optionally for facility adults; and future recipient APIs/UI should be built as one campaign-scoped recipient workspace rather than split program-specific products.
