...
export function CampaignSponsorDrawer({
  ...
  const [isDropoffLinksSectionOpen, setIsDropoffLinksSectionOpen] = useState(true);
  const [isCommunicationSectionOpen, setIsCommunicationSectionOpen] = useState(true);
  const [pendingRevokeToken, setPendingRevokeToken] = useState<CampaignSponsorDropoffToken | null>(null);
  const [isRevokingToken, setIsRevokingToken] = useState(false);
  const [generatedDropoffLink, setGeneratedDropoffLink] = useState<CampaignSponsorDropoffLink | null>(null);
  ...
  useEffect(() => {
    if (!sponsor) {
      ...
      setIsDropoffLinksSectionOpen(false);
      ...
      setIsDropoffLinksSectionOpen(true);
  ...
  const handleRevokeDropoffToken = async () => {
    if (!sponsor || !pendingRevokeToken) {
      return;
    }
    ...
    setLocalSuccess(null);
    try {
      const updated = await onRevokeDropoffToken(sponsor.id, pendingRevokeToken.id);
      if (updated) {
        onSaved(updated);
        setLocalSuccess('Sponsor drop-off QR link revoked.');
      }
      setPendingRevokeToken(null);
    } catch (revokeError) {
      setLocalError(revokeError instanceof Error ? revokeError.message : 'Unable to revoke drop-off QR link.');
    } finally {
      setIsRevokingToken(false);
    }
  };

  const handleRegenerateDropoffToken = async () => {
    if (!sponsor) {
      return;
    }
    ...
    setGeneratedDropoffLink(null);
    try {
      const result = await onRegenerateDropoffToken(sponsor.id);
      if (result) {
        onSaved(result.sponsor);
        setGeneratedDropoffLink(result.dropoffLink);
        setLocalSuccess('Fresh sponsor drop-off QR link generated.');
      }
    } catch (regenerateError) {
      setLocalError(regenerateError instanceof Error ? regenerateError.message : 'Unable to regenerate drop-off QR link.');
    } finally {
      setIsRegeneratingToken(false);
    }
  };

  const handleCopyDropoffValue = async (value: string, label: string) => {
    try {
      await navigator.clipboard.writeText(value);
      setLocalSuccess(`${label} copied.`);
    } catch {
      setLocalError(`Unable to copy ${label.toLowerCase()}.`);
    }
  };
    ...
    <>
      ...
      >
        <div className="campaign-page-stack">
          ...
          <CollapsibleSponsorSection
            title="Drop-off QR Links"
            description="Review sponsor drop-off QR links generated by sponsor emails and revoke links that should no longer work."
            ...
            action={
              canEdit && sponsor ? (
                <button
                  ...
                  disabled={isSaving || isRegeneratingToken}
                  onClick={() => void handleRegenerateDropoffToken()}
                >
                  <i className="bi bi-arrow-clockwise me-2" aria-hidden="true" />
                  {isRegeneratingToken ? 'Regenerating...' : 'Regenerate Link'}
                </button>
              ) : null
            }
            summary={
              <SponsorSummaryChips
                items={[
                  ['bi-qr-code-scan', sponsor ? `${sponsor.dropoffTokens.length} link${sponsor.dropoffTokens.length === 1 ? '' : 's'}` : 'Save sponsor first'],
                  ['bi-shield-check', `${sponsor?.dropoffTokens.filter((token) => token.status === 'ACTIVE').length ?? 0} active`],
                  ['bi-clock-history', `Last scan ${formatShortDate(sponsor?.dropoffTokens[0]?.latestScanAt ?? null)}`],
                ]}
              />
            }
          >
            ...
            {!sponsor ? (
              <div className="campaign-studio__empty-note">Save the sponsor first to generate drop-off QR links from sponsor emails.</div>
            ) : (
              <div className="campaign-sponsor-dropoff-stack">
                {generatedDropoffLink ? (
                  <div className="campaign-sponsor-dropoff-diagnostic" role="status">
                    <div>
                      <strong>Fresh drop-off link generated</strong>
                      <span>Expires {formatSponsorDateTime(generatedDropoffLink.expiresAt)}</span>
                    </div>
                    <div className="campaign-sponsor-dropoff-diagnostic__row">
                      <span>{generatedDropoffLink.dropoffUrl}</span>
                      <button
                        type="button"
                        className="btn btn-outline-secondary btn-sm"
                        onClick={() => void handleCopyDropoffValue(generatedDropoffLink.dropoffUrl, 'Drop-off URL')}
                      >
                    ...
                    <div className="campaign-sponsor-dropoff-diagnostic__row">
                      <span>{generatedDropoffLink.qrImageUrl}</span>
                      <button
                        type="button"
                        className="btn btn-outline-secondary btn-sm"
                        onClick={() => void handleCopyDropoffValue(generatedDropoffLink.qrImageUrl, 'QR image URL')}
                      >
                        <i className="bi bi-copy me-2" aria-hidden="true" />
                        Copy QR URL
                      </button>
                ...
                {sponsor.dropoffTokens.length === 0 ? (
                  <div className="campaign-studio__empty-note">No sponsor drop-off QR links have been generated yet.</div>
                ) : (
                  <div className="table-responsive">
                    <table className="table campaign-team-table align-middle">
                      <thead>
      ...
      <ConfirmationModal
        ...
        title="Revoke sponsor drop-off QR link?"
        message="This will immediately stop this QR link from opening the sponsor drop-off list. Emails already sent with this QR code will no longer work."
        details={[
          sponsor?.displayName ?? 'Sponsor',
          `Created ${formatSponsorDateTime(pendingRevokeToken?.createdAt ?? null)}`,
          `Scanned ${pendingRevokeToken?.scanCount ?? 0} time${pendingRevokeToken?.scanCount === 1 ? '' : 's'}`,
        ]}
        confirmLabel="Revoke Link"
        isSubmitting={isRevokingToken}
        onConfirm={handleRevokeDropoffToken}
...
function toDropoffTokenStatusLabel(status: CampaignSponsorDropoffToken['status']): string {
  if (status === 'ACTIVE') return 'Active';
  if (status === 'REVOKED') return 'Revoked';
  return 'Expired';
}

function dropoffTokenIcon(status: CampaignSponsorDropoffToken['status']): string {
